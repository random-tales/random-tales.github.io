...
mermaid: true
---
_Transformers revolutionized sequence modeling with their attention-based, non-recurrent architecture. This tutorial builds up the transformer model step by step—from self-attention and positional encoding to encoder-decoder attention and multi-head mechanisms—explaining each concept with math, visuals, and practical insights. Ideal for readers seeking a solid, intuitive grasp of how transformers really work under the hood._
...
![Figure 1](/assets/img_posts/transformer/first_glimpse.png){:width="100%"}
_Figure 1: First glimpse of the model._


By taking a closer look at the transformer encoder in [Figure 2](/assets/img_posts/transformer/encoder.png) we notice that the words (represented as embeddings) flow in parallel.
...
![Figure 2](/assets/img_posts/transformer/encoder.png){:width="80%"}
_Figure 2: Transformer encoder._


### Self-attention
For each embedding we compute query, key and value, by multiplying with the trainable parameters $\vW_q$, $\vW_k$, and $\vW_v$, respectively. As illustrated in [Figure 3](/assets/img_posts/transformer/qkv.png) we can see that the parameters are shared between the different embeddings.

![Figure 3](/assets/img_posts/transformer/qkv.png){:width="50%"}
_Figure 3: Computing queries, keys, and values._

Afterward, we proceed to compute self-attention. As depicted in [Figure 4](/assets/img_posts/transformer/self_attn.png), for each embedding, we calculate attention scores by multiplying the corresponding query with the keys. These scores are then transformed into attention weights through a softmax operation.

Then the new embeddings are obtained as a weighted sum of the values.
![Figure 4](/assets/img_posts/transformer/self_attn.png){:width="70%"}
...
![Figure 5](/assets/img_posts/transformer/example.png){:width="30%"}
...
To account for the order of the words we add a vector with a specific pattern to each input embedding, see [Figure 6](/assets/img_posts/transformer/pos_enc.png). 

![Figure 6](/assets/img_posts/transformer/pos_enc.png){:width="50%"}
_Figure 6: Positional encoding overview._

The positional encoding for each embedding is obtained using the formula below:

![formula](/assets/img_posts/transformer/formula.png){:width="40%"}
...
![Figure 7](/assets/img_posts/transformer/pos-enc-example.png){:width="80%"}
_Figure 7: Positional encoding example._

If we then construct a matrix that has the $pos$ index on the y-axis and the $i$ index on the x-axis we can observe the pattern in [Figure 8](/assets/img_posts/transformer/pos-enc-pattern.png).

![Figure 8](/assets/img_posts/transformer/pos-enc-pattern.png){:width="70%"}
...
To have an intuition we can think about binary numbers. In [Figure 8](/assets/img_posts/transformer/binary.png), we can see that numbers closer to each other share more of the most significant digits. Similarly, with positional encoding, position indices that are close to each other share the values of most of the indices of the embedding dimension.

![Figure 8](/assets/img_posts/transformer/binary.png){:width="70%"}
...
In [Figure 9](/assets/img_posts/transformer/transformer_decoding.gif) we see how the transformer predicts the sentence word by word[^1]. In the final layer of the decoder the softmax is used to take the word with the highest probability.


![Figure 9](/assets/img_posts/transformer/transformer_decoding.gif){:width="70%"}
...
If we take a closer look at the encoder block[^2] we see that it contains also residual connections and layer normalization.

![Figure 10](/assets/img_posts/transformer/res-conn.png){:width="60%"}
...
![Figure 11](/assets/img_posts/transformer/layernorm.png){:width="40%"}
_Figure 11: LayerNorm vs. BatchNorm._

In [Figure 11](/assets/img_posts/transformer/layernorm.png) we see the differences between LayerNorm and BatchNorm. In essence LayerNorm is preferred for the following reasons:
...
This procedure is illustrated in [Figure 12](/assets/img_posts/transformer/mha.png).

![Figure 12](/assets/img_posts/transformer/mha.png){:width="60%"}
